...
class resource_map_base
{
	...
	double measure_zeta(int configurations=100);
	double log_linear_fit_slope(std::vector<std::vector<double> > data);

	void move_n_random_points(int n);
	double set_zeta(double zeta_target, double zeta_tolerance=0.1L, int output=1);
	...
	int regrow(int number_of_sites_to_regrow);
...
class resource_map : public resource_map_base
{
	...
	resource_map();
	resource_map(int num_patches, double size=1.L, char distribution='u');
...
class resource_map_Brownian : public resource_map_base
{
	...
	int _num_patches;
	resource_map_Brownian(int num_patches=10000, double step_size_resource=0.01L);
...
class resource_map_fractal : public resource_map_base
{
	...
	void fill_fractal(int levels, int fill, int subdivide, std::vector <std::vector <double> > &position, double x_start[], double x_end[], int seed);
	resource_map_fractal(int levels, int fill, int subdivide, double size=1.L);
